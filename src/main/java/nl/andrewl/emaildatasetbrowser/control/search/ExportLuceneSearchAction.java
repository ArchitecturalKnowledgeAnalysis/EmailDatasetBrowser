...
public class ExportLuceneSearchAction implements ActionListener {
    ...
    private void writeExport(List<EmailEntryPreview> emails, EmailRepository repo, TagRepository tagRepo, String query, Path file, Consumer<String> messageConsumer) throws IOException {
        try (PrintWriter p = new PrintWriter(new FileWriter(file.toFile()), false)) {
            ...
            p.println("Tags: " + tagRepo.findAll().stream().map(Tag::name).collect(Collectors.joining(", ")));
            ...
            for (int i = 0; i < emails.size(); i++) {
                ...
                p.println("Email #" + (i + 1));
                exportEmail(email, repo, tagRepo, p, 0);
            }
        }
    }

    private void exportEmail(EmailEntryPreview email, EmailRepository repo, TagRepository tagRepo, PrintWriter p, int indentLevel) {
        ...
        p.println(indent + "Tags: " + tagRepo.getTags(email.id()).stream().map(Tag::name).collect(Collectors.joining(", ")));
        ...
        if (!replies.isEmpty()) {
            ...
            for (int i = 0; i < replies.size(); i++) {
                ...
                exportEmail(reply, repo, tagRepo, p, indentLevel + 1);
                p.println();
            }
        }
        p.println();
